...
- (bool) loadModel: (NSString *) filename {
    ...
    // Read texture coordinates -- IGNORED
...
- (bool) initShaders {
    ...
    self.objMtlProgramID = [SampleApplicationShaderUtils createProgramWithVertexShaderFileName:@"Lighting.vertsh"
                                                                   fragmentShaderFileName:@"Lighting.fragsh"];
...
- (bool) renderWithModelView:(float[]) modelViewMatrix modelViewProjMatrix:(float[]) modelViewProjMatrix
{
    ...
    if(self.isLoaded) {
        ...
        // void glUniformMatrix4fv (GLint location, GLsizei count, GLboolean transpose, const GLfloat* value)  __OSX_AVAILABLE_STARTING(__MAC_NA,__IPHONE_3_0);
...
//        Matrix.transposeM(normalMatrix, 0, inverseMatrix, 0);
        ...
        glUniform4f(self.objMtlLightColorHandle, 0.5f, 0.5f, 0.5f, 1.0f);
